...
export default function FileListPage() {
  ...
  const fetchList = async () => { //목록 가져오기 함수
	try{
		...
		/*setFileLists(
			Array.isArray(data.fileList.list)
				? data.fileList.list		//itemType 'directory'도 같이 오면 .filter()
					.filter(it => it.itemType === 'file')
					.map(it => it.fileDTO) //파일DTO만
					.filter(Boolean) //null 방어
			:[]
		);*/
		setFileLists(Array.isArray(data.fileList?.list) ? data.fileList.list : []);
		setMaxPageNum(data.fileList.maxPageNum);
		...
		//옵션
		//setFiles(data.files);
		setFiles(Array.isArray(data.fileList?.list) ? data.fileList.list : []);
 ...
 /* const formatDate = (date: Date) => {
  ...
  const formatDate = (raw?: string | Date) => {
  	if (!raw) return '-';                                // null / undefined 방어
    const d = raw instanceof Date ? raw : new Date(raw); // 문자열 → Date
                ...
                {viewMode === 'table' ? (
                  <table className="w-full">
                    ...
                    {/*<tbody className="divide-y divide-gray-200">
                      {(fileLists ?? []).map((item) => (
                        <tr key={item.pathId} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center space-x-3">
                              <i className={`${file.itemType === 'folder' ? 'ri-folder-fill text-yellow-500' : 'ri-file-text-fill text-gray-400'} text-lg`}></i>
									    ...
									    {(fileLists ?? []).map((item, idx) => {
											const key =
											      item.itemType === 'directory'
											        ? `dir_${item.pathId}`                // 폴더
											        : `file_${item.FileDTO?.fileId ?? idx}`; // 파일 (fileId 없으면 idx 백업)
									      /* 아이콘과 이름 결정 */
									      const isDir  = item.itemType === 'directory';
									      const icon   = isDir ? 'ri-folder-fill text-yellow-500'
									                           : 'ri-file-text-fill text-gray-400';
									      /*const name   = isDir
									        ? (item.path?.split('/').pop() || '-')      // 폴더명 = path 끝부분
									        : (item.FileDTO?.fileName   || '-');

									       파일 전용 정보 
									      const sizeKB = !isDir && item.FileDTO?.size != null
									        ? `${(item.FileDTO.size / 1024).toFixed(1)} KB`
									        : '-';

									      const modAt  = !isDir && item.FileDTO?.updateTime
									        ? formatDate(item.FileDTO.updateTime)
									        : '-';*/
											
											//const isDir = item.itemType === 'directory';
											const name  = isDir
											  ? item.path.split('/').pop()           // 폴더명
											  : item.fileDTO?.fileName ?? '-';
									      ...
									      /* key 는 pathId(폴더) - fileId(파일) 아무거나 유일한 값 */
									      const rowNum = isDir ? item.pathId : item.FileDTO?.fileId || item.pathId;
					  ...
					  {(fileLists ?? []).map((item, idx) => {
						const key =
							item.itemType === 'directory'
								...
								: `file_${item.FileDTO?.fileId ?? idx}`; // 파일 (fileId 없으면 idx 백업)
					    const isDir = item.itemType === 'directory';
					    //const key   = isDir ? item.pathId : item.FileDTO?.fileId || item.pathId;
					    const icon  = isDir ? 'ri-folder-fill text-yellow-500'
					                        : 'ri-file-text-fill text-gray-400';
					    /*const name  = isDir
					      ? (item.path?.split('/').pop() || '-')            // 폴더명
					      : (item.FileDTO?.fileName || '-');                // 파일명*/
						const name  = isDir
						  ? item.path.split('/').pop()           // 폴더명
						  : item.fileDTO?.fileName ?? '-';
					    /*const size  = !isDir && item.FileDTO?.size != null
					      ? `${(item.FileDTO.size / 1024).toFixed(1)} KB`
					      : '-';*/
						const size = !isDir && item.fileDTO?.size != null
						  ? `${(item.fileDTO.size / 1024).toFixed(1)} KB`
