...
function SelectHourMin({ hour, min, setHour, setMin }: SelectHourMinProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      <select value={hour} onChange={(e) => setHour(Number(e.target.value))} className="px-3 py-2 border rounded">
        {hours.map(h => <option key={h} value={h}>{h}</option>)}
      </select>
      <span className="text-sm text-gray-700">시간</span>

      <select value={min} onChange={(e) => setMin(Number(e.target.value))} className="px-3 py-2 border rounded">
...
/*  const [extTxt, setExtTxt] = useState<boolean>(false);
  ...
  // API base handling (apiUrl helper or fallback)
  const API_BASE = (typeof apiUrl === 'function') ? apiUrl('') : '';
  ...
  // -----------------------
  // 헬퍼: DTO에서 확장자 정보 추출 -> boolean 상태들로 채움
  // - 서버가 boolean, 'Y'/'N', 'true'/'false', 또는 csv string(".pdf,.txt") 중 어떤걸 보내더라도 처리
  // -----------------------
/*  function parseExtensions(dto: ChatbotDTO) {
    // 우선 개별 boolean 필드가 true/false 형태인지 확인
    const toBool = (v: any) => {
      if (v === true || v === 'true' || v === 'Y' || v === 'Y' || v === 'y' || v === '1' || v === 1) return true;
      if (v === false || v === 'false' || v === 'N' || v === 'n' || v === '0' || v === 0) return false;
      return null; // 알 수 없음
    };

    // 1) 개별 필드 우선 사용
    const bTxt = toBool(dto.txt);
    const bPdf = toBool(dto.pdf);
    const bDocx = toBool(dto.docx);
    const bXlsx = toBool(dto.xlsx);
    const bPptx = toBool(dto.pptx);
    const bHtml = toBool(dto.html);
    const bCsv = toBool(dto.cvs ?? dto.fileExtensions ?? dto.csv ?? null); // some APIs use cvs/csv

    // 2) CSV 형태(" .pdf,.txt ")로 온다면 파싱
    const fileExtCsv = typeof dto.fileExtensions === 'string' ? dto.fileExtensions : null;
    // normalize helper: remove spaces, lowercase, ensure leading dot
    const normalizeList = (s: string) =>
      s.split(',').map(x => x.trim().toLowerCase()).filter(Boolean);

    const extList = fileExtCsv ? normalizeList(fileExtCsv.replace(/\s/g, '')) : [];

    // 3) 최종 설정: 개별 boolean 값이 null(불명)일 때 csv 리스트를 기준으로 결정
    setExtTxt(bTxt ?? extList.includes('.txt') ?? false);
    setExtPdf(bPdf ?? extList.includes('.pdf') ?? false);
    setExtDocx(bDocx ?? extList.includes('.docx') ?? false);
    setExtXlsx(bXlsx ?? extList.includes('.xlsx') ?? false);
    setExtPptx(bPptx ?? extList.includes('.pptx') ?? false);
    setExtHtml(bHtml ?? extList.includes('.html') ?? false);
    setExtCsv(bCsv ?? extList.includes('.csv') ?? false);
  }*/
...
/*      txt: extTxt,
  ...
  async function runEmbedding(){
	...
	try{
		...
		try{
			//텍스트가 JSON이면 파싱
			data = text ? JOSN.parse(text): text;
		} catch (e) {
		...
		if (!res.ok) {
			//실패: 상태 코드와 메시지 함께 표시
			const errMsg = typeof data === 'string' ? data: JOSN.stringigy(data);
			setEmbeddingResultMessage(`요청 실패: ${res.status} ${errMsg}`);
			console.error('임베딩 실패', res.status, data);
		} else {
			//성공 : 서버가 준 응답을 표시
			const successText = (typeof data === 'string') ? data : JSON.stringfiy(data);
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col">
        <AdminHeader title="텍스트 임베딩 설정" onToggleSidebar={() => setIsSidebarOpen(!isSidebarOpen)} />
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="bg-white rounded-xl border border-gray-200 min-h-full">
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-6">파일의 텍스트를 추출해서 DB에 저장하기 위한 기본 설정을 합니다.</p>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex">
                    <input type="text" value={selectedPath} readOnly className="flex-1 px-3 py-2 border rounded-l bg-gray-50" />
                    <button onClick={() => setIsPathModalOpen(true)} className="px-4 py-2 bg-indigo-600 text-white rounded-r">경로 검색</button>
                  </div>
                </div>
                ...
                {/* 확장자 체크박스 (원래 UI 복원) */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-3">텍스트 임베딩 할 확장자 (미구현 기능)</label>
                  <div className="grid grid-cols-4 gap-4">
                    <label className="flex items-center"><input type="checkbox" /*checked={extTxt} onChange={(e) => setExtTxt(e.target.checked)}*/ className="mr-2" /><span className="text-sm">txt</span></label>
                    <label className="flex items-center"><input type="checkbox" /*checked={extPdf} onChange={(e) => setExtPdf(e.target.checked)}*/ className="mr-2" /><span className="text-sm">pdf</span></label>
                    <label className="flex items-center"><input type="checkbox" /*checked={extDocx} onChange={(e) => setExtDocx(e.target.checked)}*/ className="mr-2" /><span className="text-sm">docx</span></label>
                    <label className="flex items-center"><input type="checkbox" /*checked={extXlsx} onChange={(e) => setExtXlsx(e.target.checked)}*/ className="mr-2" /><span className="text-sm">xlsx</span></label>

                    <label className="flex items-center"><input type="checkbox" /*checked={extPptx} onChange={(e) => setExtPptx(e.target.checked)}*/ className="mr-2" /><span className="text-sm">pptx</span></label>
                    <label className="flex items-center"><input type="checkbox" /*checked={extHtml} onChange={(e) => setExtHtml(e.target.checked)}*/ className="mr-2" /><span className="text-sm">html</span></label>
                    <label className="flex items-center"><input type="checkbox" /*checked={extCsv} onChange={(e) => setExtCsv(e.target.checked)}*/ className="mr-2" /><span className="text-sm">csv</span></label>
                    <label className="flex items-center"><input type="checkbox" className="mr-2" /><span className="text-sm">기타</span></label>
                  </div>
                </div> <hr/>
              ...
              <div className="flex justify-end mt-8">
                <button onClick={handleSave} className="px-6 py-2 bg-indigo-600 text-white rounded">설정 저장</button>
              </div>
      ...
      {isPathModalOpen && <PathSearchModal isOpen={isPathModalOpen} onClose={() => setIsPathModalOpen(false)} onSelect={handlePathSelect} />}
    </div>
  );
}
